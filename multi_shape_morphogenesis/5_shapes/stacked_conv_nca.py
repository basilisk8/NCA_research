...
Architecture:
 ...
 - Stacked conv approach (no fixed Sobel filters):
   - Conv1: 25 to 128 channels, 3x3 kernel (perceive neighbors + begin thinking)
   - Conv2: 128 to 128 channels, 1x1 kernel (deeper per-cell processing)
   - Conv3: 128 to 21 channels, 1x1 kernel, zero-initialized (decide update)
...
conv1 = nn.Conv2d(25, 128, kernel_size=3, padding=1).to(device)  # perceive + think
conv2 = nn.Conv2d(128, 128, kernel_size=1).to(device)             # think deeper
conv3 = nn.Conv2d(128, 21, kernel_size=1).to(device)              # decide update
...
# Precompute target shapes (binary, 40x40)
targets = []

# Shape 0: Circle (radius 10, centered at 20,20)
...
# Shape 3: Triangle (pointing down, base at top area)
t = torch.zeros(40, 40, device=device)
...
if __name__ == "__main__":
    ...
    }, 'stacked_conv_weights.pth')
    print("Weights saved to stacked_conv_weights.pth")